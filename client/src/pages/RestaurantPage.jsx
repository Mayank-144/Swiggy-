  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 pb-28">
      ...
      <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 mb-4">
        ...
        <span>/</span>
        <span className="text-slate-600 truncate">{restaurant.location?.city || 'Bengaluru'}</span>
        <span>/</span>
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400 font-medium">
              ...
              <span>{restaurant.location?.address || `${restaurant.location?.area}, ${restaurant.location?.city}`}</span>
            </div>
          </div>

          {/* Rating Block */}
          <div className="flex sm:flex-col items-center sm:items-end justify-between p-3 rounded-2xl bg-slate-50 border border-slate-200/70 shrink-0">
            <div className="flex items-center gap-1 text-emerald-700 font-black text-base">
        ...
        <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-xs sm:text-sm font-extrabold text-slate-700">
        ...
        {/* Deals & Offers Banner */}
        {restaurant.discount && (
          <div className="p-3.5 rounded-2xl bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-rose-500/10 border border-amber-300/60 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-xs font-bold text-amber-900">
              <Tag className="w-4 h-4 text-swiggy-orange" />
              <span>{restaurant.discount}</span>
            </div>
            {restaurant.discountCode && (
              <span className="px-2.5 py-1 rounded-lg bg-white font-mono text-[11px] font-black text-amber-800 border border-amber-200 shadow-2xs">
      ...
      <div className="my-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 sticky top-20 z-30 bg-[#FAFAFC]/95 backdrop-blur-md py-3">
        {/* Veg Only Toggle */}
      ...
      <div className="space-y-6">
        {restaurant.menuCategories?.map((category) => {
          ...
          return (
            <div
              key={category.name}
              className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm overflow-hidden"
            >
              {/* Accordion Header */}
              <button
                onClick={() => toggleCategory(category.name)}
                className="w-full flex items-center justify-between font-black text-lg text-slate-800 tracking-tight pb-2"
              >
                <span>
      ...
      {totalItemsCount > 0 && (
        <motion.div
          ...
          className="fixed bottom-5 inset-x-4 max-w-xl mx-auto z-40"
        >
          <button
            onClick={openCartDrawer}
            className="w-full bg-slate-900 text-white p-4 rounded-2xl shadow-2xl flex items-center justify-between font-black text-sm hover:bg-black transition-all border border-slate-800"
          >
            <div className="flex items-center gap-3">
              <div className="p-2 bg-swiggy-orange rounded-xl text-white">
            ...
            <div className="flex items-center gap-2 text-swiggy-orange uppercase tracking-wider text-xs font-black">
              <span>View Cart</span>
              <span>→</span>
            </div>
          </button>
