            ...
            >
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 px-4 rounded-xl bg-swiggy-orange text-white font-bold text-xs hover:bg-swiggy-orangeDark shadow-md transition-colors"
                >
                  Yes, Start Fresh
                </button>
              </div>
            ...
            >
              ...
              <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-2.5">
                  <div className="p-2 bg-swiggy-orange/10 text-swiggy-orange rounded-xl">
                    <ShoppingBag className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="font-extrabold text-base text-slate-800 tracking-tight">Your Cart</h3>
                    {restaurant && (
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-6">
                {cartItems.length === 0 ? (
                  <div className="py-16 text-center space-y-4">
                    <div className="w-24 h-24 mx-auto rounded-full bg-slate-100 flex items-center justify-center text-slate-300">
                      <ShoppingBag className="w-12 h-12 stroke-[1.5]" />
                    </div>
                    <div>
                      <h4 className="font-extrabold text-lg text-slate-700">Your cart is empty</h4>
                    ...
                    <button
                      onClick={closeCartDrawer}
                      className="px-6 py-2.5 bg-swiggy-orange hover:bg-swiggy-orangeDark text-white font-bold text-xs rounded-xl shadow-md transition-all uppercase tracking-wider"
                    >
                  ...
                  <>
                    ...
                    <div className="space-y-4">
                      {cartItems.map((item) => (
                        <div
                          key={item.id}
                          className="flex items-center justify-between gap-3 p-3 rounded-2xl bg-slate-50/70 border border-slate-100 hover:border-slate-200 transition-colors"
                        >
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <div className="bg-white border border-slate-200 rounded-lg shadow-sm flex items-center px-1.5 py-1">
                              <button
                                onClick={() => updateQuantity(item.id, -1)}
                                className="p-0.5 text-slate-600 hover:text-emerald-600 transition-colors"
                              >
                            ...
                            <span className="font-extrabold text-xs text-slate-800 w-12 text-right">
                    ...
                    {/* Coupons Section */}
                    <div className="p-4 rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200/60 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        {appliedCoupon ? (
                          ...
                          <button
                            ...
                            className="text-xs font-extrabold text-swiggy-orange hover:underline"
                          >
                      ...
                      {appliedCoupon ? (
                        <div className="p-2.5 rounded-xl bg-white border border-emerald-300 text-emerald-700 flex items-center justify-between text-xs font-bold shadow-sm">
                        ...
                        showCouponBox && (
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-swiggy-orange hover:bg-swiggy-orangeDark text-white font-bold text-xs rounded-xl transition-colors shadow-sm"
                            >
                              Apply
                            </button>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-1.5 pt-1">
                          {Object.entries(availableCoupons).map(([code, meta]) => (
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-white border border-amber-300/80 text-[10px] font-extrabold text-amber-900 hover:bg-amber-100 transition-colors shadow-2xs"
                            >
                    ...
                    <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        {tip > 0 && (
                          <button onClick={() => setTip(0)} className="text-[11px] text-slate-400 hover:text-slate-600">
                            Clear
                          </button>
                        )}
                      </div>
                        ...
                        {[10, 20, 30, 50].map((amount) => (
                          <button
                            ...
                            className={`flex-1 py-1.5 rounded-xl text-xs font-bold border transition-all ${
                              tip === amount
                                ? 'bg-swiggy-orange text-white border-swiggy-orange shadow-sm'
              ...
              {cartItems.length > 0 && (
                <div className="p-5 border-t border-slate-100 bg-slate-50/50 space-y-2">
                  <button
                    ...
                    className="w-full py-3.5 px-4 bg-swiggy-orange hover:bg-swiggy-orangeDark text-white font-extrabold text-sm rounded-2xl shadow-xl shadow-swiggy-orange/30 flex items-center justify-between transition-all active:scale-[0.99] uppercase tracking-wider"
                  >
